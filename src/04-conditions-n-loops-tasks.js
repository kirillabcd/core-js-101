...
function isTriangle(a, b, c) {
  const resultArr = [a, b, c].sort((first, second) => first - second);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    for (let j = 0; j < config.length; j += 1) {
      stack = stack.replace(config[j][0] + config[j][1], '');
    }
  }
  if (stack.length === 0) {
    return true;
...
function getCommonDirectoryPath(pathes) {
  const firstSegment = pathes[0].split('/');
  for (let i = 1; i < pathes.length; i += 1) {
    const currentSegment = pathes[i].split('/');
    for (let j = 0; j < currentSegment.length; j += 1) {
      if (firstSegment[j] !== currentSegment[j]) {
        firstSegment.splice(j);
      }
    }
  }
  return firstSegment.length > 0 ? `${firstSegment.join('/')}/` : '';
}
